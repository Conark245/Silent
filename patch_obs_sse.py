...
sse_old = """      eventSource.addEventListener('donation_approved', (e: any) => {
        try {
...
sse_new = """      eventSource.addEventListener('donation_approved', (e: any) => {
        try {
          const event: DonationEvent = JSON.parse(e.data);
          if (event && event.eventId && !processedEventIdsRef.current.has(event.eventId)) {
            console.log('[OBS Overlay] New donation event received:', event);
      ...
      eventSource.addEventListener('theme_updated', (e: any) => {
        try {
